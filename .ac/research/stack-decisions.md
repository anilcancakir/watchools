...
No player exposes a resolver hook. ExoPlayer, AVPlayer, libmpv and hls.js each resolve independently. The full mpv option manual has no `dns` or `resolve` option, and `--http-proxy` is not used for https URLs. The hook is genuinely missing, but the conclusion drawn from it does not follow: pinning an address does not need a hook, only a caller that reaches the request before it goes out. `HttpOverrides` reaches every request this app makes over its own HTTP client, and that is as far as an in-app resolver decision goes. It does not reach playback, and the reason is the player rather than the redirect. A stream URL is built on the panel's own host (`lib/app/protocol/xtream/xtream_stream_url.dart:113-122`), so playback's FIRST request already goes to the name the user configured, and libmpv resolves it through `getaddrinfo` inside FFmpeg before a redirect exists. The `302` to another origin (`.ac/research/player-layer.md:26-28`) widens the gap, because that second host is one the app never sees, and `libavformat/http.c:487-509` replaces `s->location` and jumps to `redo` while `s->headers` survives untouched; but the redirect is not the cause and an earlier version of this paragraph said it was.
...
**Decision: `HttpOverrides` covers the app's own HTTP, and that is the whole reach of an in-app resolver.** Playback resolves every address itself, the panel's included, so the user-visible consequence is that the catalogue can start working while a channel still does not; the settings screen says exactly that. A setup screen per platform still carries the case neither mechanism reaches.
