...
///
/// The focus treatment is Apple's, implemented rather than approximated. tvOS
/// grows a focused item by a constant 70 points on its largest dimension
/// (`1.0 + 70/max(w,h)` in Re-Lax, which reproduces Apple's published Top Shelf
/// focused and unfocused sizes exactly) and drops a 40 point blur 50 points
/// below it. On a 120 pixel tile that is a 1.58 lift, so the gutters have to
/// carry 35 points a side: Apple's own grid spec asks for 40 horizontal and 100
/// vertical for precisely this reason. Here the tile is large enough that the
/// lift stays sane, and it is worth seeing what the real constant looks like
/// before deciding whether this product wants it.
@immutable
class MosaicLayout extends StatelessWidget {
  ...
  /// The unfocused tile edge. Chosen so the lift lands near Apple's five
  /// column card (320 pt grows 22%); a smaller tile makes the constant lift
  /// proportionally violent.
  static const double tile = 132;

  /// tvOS's constant: 70 points on the largest dimension, whatever the size.
  static const double focusLift = 1.0 + 70.0 / tile;
  ...
  Widget _grid() {
    ...
    return GridView.builder(
      ...
      gridDelegate: const SliverGridDelegateWithMaxCrossAxisExtent(
        ...
        // 40 horizontal is Apple's number and it exists to absorb the lift.
        crossAxisSpacing: 20,
        mainAxisSpacing: 20,
            ...
            child: AnimatedScale(
              ...
              child: DecoratedBox(
                decoration: BoxDecoration(
                  ...
                  boxShadow: <BoxShadow>[
                    // 25% black, 5 blur, 4 down unfocused; 52.5%, 40 blur, 50
                    // down focused. Re-Lax's values, and the part of the effect
                    // that does most of the perceptual work.
                    BoxShadow(
                      color: Color(selected ? 0x86000000 : 0x40000000),
                      blurRadius: selected ? 40 : 5,
                      offset: Offset(0, selected ? 50 : 4),
                    ),
                  ],
                ),
                child: AspectRatio(
