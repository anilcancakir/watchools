...
- **The provider faults are produced and wired now, and the vocabulary is four rather than three.** `ProviderFault` (`lib/app/models/provider_fault.dart`) carries `unreachable`, `expired`, `throttled` and `evicted`, `classifyProviderFault` is the single reader, and `ProviderNotice` renders them where `GuideEmpty` goes. Only `expired` withholds the retry, since retrying a lapsed subscription fails identically every time, and `/saglayici` renders `expired` as a sentence naming the two fields instead of through the panel, because there the settings are already in front of the user. **What is missing is a fifth member**: the classifier consults `statusCode` only for `== 0` (`xtream_account.dart:211`), so a 301, 403, 404 or 502 falls through to `throttled` and tells a user who mistyped the port that they are rate limited and should wait a few seconds, forever. **Loading is still open**, deliberately: it is a skeleton of the layout it replaces rather than a message, so it is three per-surface pieces rather than one shared component, and none is built.
