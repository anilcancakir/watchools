...
No player engine exposes a resolver hook, but pinning an address does not need one. `HttpOverrides` reaches every request this app makes over its own HTTP client, which is the catalogue. **It does not reach playback, and the reason is the player rather than the redirect.** A stream URL is built on the panel's own host (`lib/app/protocol/xtream/xtream_stream_url.dart:113-122`), so playback's FIRST request already goes to the name the user configured, and libmpv resolves it through `getaddrinfo` inside FFmpeg before a redirect exists at all. The `302` to another origin (`.ac/research/player-layer.md:26-28`) makes the gap wider, since that second host is one the app never sees, but it is not what causes it. The consequence to state to a user is the specific one: the catalogue can start working while a channel still does not. An earlier version of this paragraph blamed the redirect, and `lib/ui/layouts/provider_settings_layout.dart`'s own doc block records the correction.
...
The engine observes `AppLifecycleState.paused` and branches on a per-credential `BackgroundPlayback` (`lib/app/models/background_playback.dart`): `stop` tears the core down and releases the wakelock, while `audio` and `pictureInPicture` release only the wakelock. `paused` is delivered on iOS and Android only, stated verbatim at `sky_engine/lib/ui/platform_dispatcher.dart:2444` ("This state is only entered on iOS and Android"), which is why no platform check is written anywhere: the absence is the mechanism, not an oversight. `audio` and `pictureInPicture` share one Dart arm because the difference between them is platform work no target has yet, so each later engine implementation needs its own observer. `resumed` deliberately does nothing: an automatic reload retakes the account's single connection slot (measured `max_connections: 1`) and evicts whichever device picked it up while the user was away, which is the harm stopping on background exists to prevent. The setting is written by `ProviderSession.setBackgroundPlayback`, not by `submit`, because `submit` only stores past a live panel handshake and the credential fields start empty on every open of `/saglayici`, so riding it would mean a user could not change this without retyping their whole credential against a reachable panel.
