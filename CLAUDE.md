...
- **The provider faults are designed and nothing produces one.** `ProviderFault` (`lib/app/models/provider_fault.dart`) is the vocabulary the Xtream client will report in, and `ProviderNotice` renders all three: unreachable, expired, throttled. Three rather than one error because they are opposite from where the user sits, and only `expired` withholds the retry, since retrying a lapsed subscription fails identically every time. It goes where `GuideEmpty` goes, inside the body branch under the pinned toolbar, so wiring it is one arm of an existing conditional per surface once a controller can hold a fault. Until then the panel lives at `/preview` alone; the controllers stay `SimpleMagicController` because their own doc says the state model waits for the client. **Loading is still open**, deliberately: it is a skeleton of the layout it replaces rather than a message, so it is three per-surface pieces rather than one shared component, and none is built.
